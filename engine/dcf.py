...
def calculate_dcf(fcf, growth_rate, wacc, terminal_growth, years,
                  shares_outstanding, net_debt):
    if fcf <= 0:
        fcf = abs(fcf) if fcf != 0 else 1.0
    if wacc <= terminal_growth:
        terminal_growth = wacc - 0.01

    fcf_projections, pv_fcfs = [], []
    for i in range(1, years + 1):
        proj = fcf * ((1 + growth_rate) ** i)
        pv   = proj / ((1 + wacc) ** i)
    ...
    intrinsic         = equity_value / shares_outstanding if shares_outstanding > 0 else 0.0
...
def sensitivity_analysis(fcf, base_growth, base_wacc, terminal_growth, years,
                         shares_outstanding, net_debt):
    growth_range = [max(0.01, base_growth + d) for d in [-0.04, -0.02, 0, 0.02, 0.04]]
    wacc_range   = [max(0.02, base_wacc   + d) for d in [-0.02, -0.01, 0, 0.01, 0.02]]
    results = {}
    for w in wacc_range:
        col = f"{w*100:.0f}%"
        col_data = {}
